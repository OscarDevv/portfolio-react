...
function OtimizationProvider({ children }) {
  ...
  return (
    <OtimizationContext.Provider value={{avgFps, slowMode}}>
      ...
      {showSugestion && (
        <div className="fixed right-0 bottom-0 bg-slate-950 border-t-2 border-l-2 border-slate-500 p-10 z-70 rounded-tl-md" ref={dialog}>
          <div className="flex justify-between items-center">
            <span>FPS atual: {avgFps}</span>
            <FontAwesomeIcon icon={faX} className="text-2xl cursor-pointer" onClick={() => setDialogDisplay(false)} />
          </div>

          <p className="my-5">
            Seu FPS está baixo, considere ativar a opção {"Modo lento"} para melhor performance.
          </p>

          <Button type="button" variant="light" onClick={() => setSlowMode(prev => !prev)}>Alternar modo lento</Button>
